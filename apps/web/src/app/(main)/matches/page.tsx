...
function MatchCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-3 px-4 py-3 text-left transition border-b border-white/5 hover:bg-white/5 ${
        active ? 'bg-white/10' : ''
      }`}
    >
...
export default function MatchesPage() {
  ...
  useEffect(() => {
    ...
    matchesApi.getMessages(activeMatchId).then(msgs => setMessages(activeMatchId, msgs));
  }, [activeMatchId]);
